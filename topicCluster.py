...
def getAverageFeatureForCharVec(charList,char2v,keys):

    feature = np.zeros(256,dtype='float32')
    count = 0
...
def getAverageFeatureForWordVec(wordList, word2v,keys):
...
def Main():
    ...
    topicCharMatrix = np.zeros((256,len(topicdf['topid'])),dtype = 'float32')   #256,1999
    topicWordMatrix = np.zeros((256,len(topicdf['topid'])),dtype = 'float32')   #256,1999
    ...
    for i in xrange(len(topicId)):
        topicCharMatrix[:,i] = getAverageFeatureForCharVec(topicCharInfor[i],char2v,charkey)
        topicWordMatrix[:,i] = getAverageFeatureForWordVec(topicWordInfor[i],word2v,wordkey)
    ...
    testCharMatrix = np.zeros((len(test['qid']),256),dtype = 'float32') #411720,256
    testWordMatrix = np.zeros((len(test['qid']),256),dtype = 'float32') #411720,256
    ...
    for i in range(len(testId)):
        ...
        # print cList[i]
        testCharMatrix[i,:] = getAverageFeatureForCharVec(testCharInfor[i], char2v,charkey)
        testWordMatrix[i,:] = getAverageFeatureForWordVec(testWordInfor[i], word2v,wordkey)

    # testCharMatrix = map(lambda x: float(x), testCharInfor)
    # topicCharInfor = map(lambda x: float(x), topicCharInfor)

    charSilimaity = np.dot(testCharMatrix,topicCharMatrix)  #411720,1999,行中代表这
    wordSilimaity = np.dot(testWordMatrix, topicWordMatrix) #411720,1999
    alpha  = 0.75
    similarity = alpha*wordSilimaity + (1-alpha)*charSilimaity
    result = []
    for i in xrange(len(testId)):
        result.append(testId[i])
        tmpSimi = dict(enumerate(similarity[i,:]))
        sortSimilry = sorted(tmpSimi.items(), key=lambda x: x[1], reverse=True) #从大到小排序
        for idx,val in sortSimilry[:5]:
            result.append(topicId[idx])

    print charSilimaity.shape,wordSilimaity.shape
    print 'writing ... '
    # print result
    # # res = []
    ...
    sub.to_csv('simplewordsilimary.csv', index=False, sep=',', header=None)
    ##write_to_txt
    endTime = time.clock()

    print 'total running time is :',endTime-startTime
            #十个聚类
